...
describe('parseTargetUrl', () => {
  ...
  it('rejects non-http(s) schemes', () => {
    for (const bad of [
      ...
      assert.throws(() => parseTargetUrl(bad), SecurityValidationError);
    }
  });

  it('rejects malformed URLs', () => {
    assert.throws(() => parseTargetUrl('not a url'), SecurityValidationError);
    assert.throws(() => parseTargetUrl(''), SecurityValidationError);
    assert.throws(() => parseTargetUrl('http://'), SecurityValidationError);
  });

  it('rejects URLs that embed credentials', () => {
    assert.throws(
      () => parseTargetUrl('http://user:pass@example.com/'),
      SecurityValidationError,
    );
    assert.throws(
      () => parseTargetUrl('http://user@example.com/'),
      SecurityValidationError,
    );
  });
});
...
describe('isNonPublicAddress', () => {
  it('rejects an invalid IP string', () => {
    assert.throws(() => isNonPublicAddress('definitely-not-an-ip'), SecurityValidationError);
  });

  it('flags loopback addresses', () => {
    assert.equal(isNonPublicAddress('127.0.0.1'), true);
    assert.equal(isNonPublicAddress('127.0.0.0'), true);
    assert.equal(isNonPublicAddress('127.255.255.255'), true);
  ...
  it('flags private IPv4 ranges', () => {
    ...
    assert.equal(isNonPublicAddress('192.168.0.1'), true);
    assert.equal(isNonPublicAddress('192.168.255.255'), true);
  });

  it('flags link-local and CGNAT ranges', () => {
    assert.equal(isNonPublicAddress('169.254.169.254'), true);
    assert.equal(isNonPublicAddress('169.254.0.1'), true);
  ...
  it('flags IPv6 private/loopback/link-local/multicast ranges', () => {
  ...
  it('flags IPv4-mapped IPv6 addresses that map to private IPs', () => {
    assert.equal(isNonPublicAddress('::ffff:127.0.0.1'), true);
    assert.equal(isNonPublicAddress('::ffff:10.0.0.1'), true);
    assert.equal(isNonPublicAddress('::ffff:192.168.1.1'), true);
    assert.equal(isNonPublicAddress('::ffff:169.254.169.254'), true);
    assert.equal(isNonPublicAddress('::ffff:7f00:1'), true);
    assert.equal(isNonPublicAddress('::ffff:0a00:0001'), true);
  });

  it('flags bracketed IPv6 addresses', () => {
    assert.equal(isNonPublicAddress('[::1]'), true);
    assert.equal(isNonPublicAddress('[fd00::1]'), true);
    assert.equal(isNonPublicAddress('[fe80::1]'), true);
    assert.equal(isNonPublicAddress('[::ffff:127.0.0.1]'), true);
  ...
  it('allows public IPv6 addresses', () => {
    ...
    assert.equal(isNonPublicAddress('2001:4860:4860::8888'), false);
  });
});
...
describe('resolvePublicTarget', () => {
  ...
  it('resolves HTTPS correctly and defaults port to 443', async () => {
    const resolver = async () => ['93.184.216.34'];
    const target = await validatePublicUrl('https://example.com/', { resolver });
  ...
  it('throws when DNS returns no addresses', async () => {
    const resolver = async () => [];
    await assert.rejects(
      validatePublicUrl('http://empty-dns.example.com/', { resolver }),
      (error: unknown) =>
        error instanceof SecurityValidationError &&
        error.code === 'DNS_FAILED',
    );
  ...
  it('blocks an ipv6 private literal', async () => {
    await assert.rejects(validatePublicUrl('http://[::1]/'), SecurityValidationError);
    await assert.rejects(validatePublicUrl('http://[fd00::1]/'), SecurityValidationError);
  });

  it('blocks an ipv4-mapped ipv6 literal', async () => {
    await assert.rejects(
      validatePublicUrl('http://[::ffff:127.0.0.1]/'),
      SecurityValidationError,
    );
  });

  it('returns the correct pinned IP for the caller to use', async () => {
    const resolver = async () => ['93.184.216.34'];
    const target = await validatePublicUrl('http://example.com/', { resolver });
    assert.equal(target.ip, '93.184.216.34');
